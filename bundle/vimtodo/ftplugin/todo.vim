...
call s:Map("Tc", "CheckboxToggle")
call s:Map("Tv", "PromptTaskState")
call s:Map("Ts", "NextTaskState")
call s:Map("Tt", "LoadTaskLink")
call s:Map("Tl", "LoadLink")
call s:Map("Ta", "ArchiveDone")
"1}}}

" Todo entry macros
" ds - Datestamp {{{1
iab ds <C-R>=strftime("%Y-%m-%d")<CR>
" Tn, \Tn - New todo entry {{{1
...
if !hasmapto(':Today')
    map <buffer> <unique> <LocalLeader>Td :Today<CR>
endif
if !hasmapto(':Tomorrow')
    map <buffer> <unique> <LocalLeader>Tf :Tomorrow<CR>
endif
if !hasmapto(':Week')
    map <buffer> <unique> <LocalLeader>Tw :Week<CR>
endif
if !hasmapto(':Overdue')
    map <buffer> <unique> <LocalLeader>Tx :Overdue<CR>
endif
"1}}}
